...
describe('/unstar command', () => {
  ...
  it('is named unstar', () => {
    expect(cmd.default.data.name).toBe('unstar')
  })

  it('spends a star and replies with a public embed', async () => {
  ...
  it('replies ephemerally when the character has no star to spend', async () => {
  ...
  it('works when the user unstars themselves', async () => {
    repo.getActiveCharacter.mockReturnValue({ id: 1, name: 'Aldric', star: true })
